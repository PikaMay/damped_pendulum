...
def plot_train_and_test_losses(metrics_history, save_dir=""):
    ...
    # Extract the losses
    train_losses = [
        loss.item() for loss in metrics_history["train_loss"]
    ]  # Convert train_loss values to scalars
    test_losses = [
        entry["loss"].item() for entry in metrics_history["test_loss"]
    ]  # Convert test_loss values to scalars
...
def plot_predictions(t, y_true, y_pred, save_dir=""):
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(
        t, y_true[:, 0], label="True θ (Ground Truth)", linestyle="--", color="blue"
    )
...
def main(cfg: DictConfig):
    ...
    # Execution time
    # execution_time = timeit.timeit(lambda: train(key, model, learning_rate, epochs, data), number=1)
    ...
    os.makedirs(cfg.output_dirs.loss_plots, exist_ok=True)
    os.makedirs(cfg.output_dirs.prediction_plots, exist_ok=True)

    # Save loss plots
    plot_train_and_test_losses(metrics_history, save_dir=cfg.output_dirs.loss_plots)
    # Save prediction vs. ground truth plot
    plot_predictions(t.flatten(), y, y_pred, save_dir=cfg.output_dirs.prediction_plots)
